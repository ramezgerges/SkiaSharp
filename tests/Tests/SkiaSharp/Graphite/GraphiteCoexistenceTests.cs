...
{
	/// <summary>
	/// US4 / FR-014 / SC-002: a single .NET process must be able to instantiate
	/// and use both <see cref="GRContext"/> (Ganesh) and
	/// <see cref="SKGraphiteContext"/> (Graphite) without one corrupting the
	/// other. Both backends share the same VkInstance/VkDevice via
	/// <see cref="VulkanLoader.Shared"/>, exercising the "two consumers, one
	/// device" path.
	///
	/// SC-002 requires pixel parity within ~1% across backends for the same
	/// scene. Lavapipe is fully software-deterministic AND both backends use
	/// the same Skia drawing primitives, so cross-backend rendering of an
	/// antialiased path is exactly identical in our experience — but we
	/// allow a generous tolerance here since cross-backend exactness is not
	/// guaranteed by the spec, and intermediate Skia versions might
	/// introduce small per-backend differences.
	/// </summary>
	public class GraphiteCoexistenceTests : BaseTest
	{
		...
		public unsafe void Coexist_GaneshAndGraphite_BothRenderSameScene ()
		{
			...
			// Pixel-by-pixel: SC-002 says <1% per-channel tolerance for "same scene"
