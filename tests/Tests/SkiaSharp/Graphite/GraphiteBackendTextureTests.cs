...
{
	/// <summary>
	/// US3 / FR-010..FR-012: wrap an externally-allocated VkImage as a
	...
	/// VkImage (caller retains ownership per FR-012).
	/// </summary>
	public unsafe class GraphiteBackendTextureTests : BaseTest
	{
		...
		{
			// T049 / FR-010 (Skia-allocated path): the Recorder allocates the
			...
			using var bt = rec.CreateBackendTexture (32, 32, info);
			Assert.NotNull (bt);
			Assert.True (bt.IsValid);
			Assert.Equal (new SKSizeI (32, 32), bt.Dimensions);
