...
public class VariableFontSample : CanvasSampleBase
{
	...
	protected override void OnDrawSample(SKCanvas canvas, int width, int height)
	{
		...
		var position = new[]
		{
			new SKFontVariationDesignPositionCoordinate { Axis = SKFontVariationTag.Make("wght"), Value = weight },
			new SKFontVariationDesignPositionCoordinate { Axis = SKFontVariationTag.Make("opsz"), Value = opticalSize },
		};
	...
	private void DrawWeightSpectrum(SKCanvas canvas, int width, int height, SKTypeface currentTypeface)
	{
		...
		for (int i = 0; i < weights.Length; i++)
		{
			...
			var pos = new[]
			{
				new SKFontVariationDesignPositionCoordinate { Axis = SKFontVariationTag.Make("wght"), Value = w },
				new SKFontVariationDesignPositionCoordinate { Axis = SKFontVariationTag.Make("opsz"), Value = opticalSize },
			};
